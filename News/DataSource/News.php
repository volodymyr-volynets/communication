...
use Numbers\Communication\News\Model\Categories;
use Numbers\Communication\News\Model\News\Language\Contents;
use Numbers\Communication\News\Model\News\Languages;
use Numbers\Communication\News\Model\News\Organizations;
use Numbers\Communication\News\Model\News\Roles;
use Numbers\Communication\News\Model\News\Users;
    ...
    public function query($parameters, $options = [])
    {
        ...
        if (!empty($parameters['include_content'])) {
            $this->query->join('LEFT', function (& $query) {
                $query = Languages::queryBuilderStatic(['alias' => 'inner_d2'])->select();
                $query->columns([
                ...
                $query->join('LEFT', new Languages(), 'inner_d1', 'ON', [
                ...
                $query->join('INNER', new Contents(), 'inner_d3', 'ON', [
        ...
        if (!empty($organizations) && !\User::get('super_admin')) {
            $this->query->where('AND', function (& $query) use ($organizations) {
                $query = Organizations::queryBuilderStatic(['alias' => 'inner_d'])->select();
                $query->columns(1);
        ...
        $this->query->where('AND', function (& $query) {
            ...
            $query->where('OR', function (& $query) {
                $query = Roles::queryBuilderStatic(['alias' => 'inner_r'])->select();
                $query->columns(1);
