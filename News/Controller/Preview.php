...
class Preview extends Authorized
{
        ...
        if (!empty($summary['summary']['Unread'])) {
            ...
            foreach ($data as $k => $v) {
                if (empty($v['ns_new_read_user'])) {
                    if (!empty($tab_options['unread'])) {
                        $tab_options['unread'] .= '<hr/>';
                    }
                    $tab_options['unread'] .= $this->helperRenderOneNews($v);
                    $update[] = [
            ...
            $update_result = Users::collectionStatic()->mergeMultiple($update);
        }
