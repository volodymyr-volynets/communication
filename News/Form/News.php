...
class News extends Base
{
    ...
    public function validate(& $form)
    {
        ...
        if (empty($form->values['\Numbers\Communication\News\Model\News\Roles']) && empty($form->values['ns_new_show_to_all_roles'])) {
            $form->error(DANGER, Messages::REQUIRED_FIELD, "\Numbers\Communication\News\Model\News\Roles[1][ns_nwsrol_role_id]");
        }
        ...
        if (empty($form->values['\Numbers\Communication\News\Model\News\Organizations']) && empty($form->values['ns_new_all_organizations'])) {
            $form->error(DANGER, Messages::REQUIRED_FIELD, "\Numbers\Communication\News\Model\News\Organizations[1][ns_nwsorg_organization_id]");
        }
